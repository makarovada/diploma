...
### Проблемные записи и потоки

Два разных типа ситуаций:

| Ситуация | Где смотреть | Что делать |
|----------|--------------|------------|
| **Проблемная запись** — одна строка не прошла нормализацию | `/issues`, вкладка «Проблемы» у подключения | Открыть деталь → «Исправить правило» / «Игнорировать» / «Отметить решённым» |
| **Сбой синка** — ошибка чтения или записи | `/runs`, обзор подключения | «Повторить» или отключить проблемный поток |

**Управление потоками** (`/connections/:id/streams`):

- **Отключить** — поток исключается из cron и ручных запусков; данные в приёмнике сохраняются.
- **Синхронизировать** — запуск только выбранного потока.
- **Включить и синхронизировать** — для ранее отключённого потока.
- **Отменить запуск** — прерывание между потоками (текущий поток дочитывается до конца).
