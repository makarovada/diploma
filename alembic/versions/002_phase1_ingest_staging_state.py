...
def upgrade() -> None:
    ...
    for tbl in staging_tables:
        ...
        if "_ingest_extracted_at" not in c:
            op.add_column(
                tbl,
                sa.Column(
    ...
    # --- sync_state: per-stream + Ingest state ---
    if "sync_state" in _tables(bind):
        ...
        if "ingest_state" not in _cols(bind, "sync_state"):
            op.add_column("sync_state", sa.Column("ingest_state", postgresql.JSONB(), nullable=True))
    ...
    # --- canonical_sales._ingest_loaded_at ---
    if "canonical_sales" in _tables(bind):
        cc = _cols(bind, "canonical_sales")
        if "_ingest_loaded_at" not in cc:
            op.add_column(
                "canonical_sales",
...
def downgrade() -> None:
    ...
    if "canonical_sales" in t and "_ingest_loaded_at" in _cols(bind, "canonical_sales"):
        op.drop_column("canonical_sales", "_ingest_loaded_at")

    if "sync_state" in t:
        op.execute(sa.text("DROP INDEX IF EXISTS uq_sync_state_integration_stream"))
        for col in ("ingest_state", "cursor_field", "sync_mode", "stream_name"):
    ...
    for new_n, old_n in staging:
        if new_n in cur and old_n not in cur:
            for ac in ("_ingest_meta", "_ingest_extracted_at", "_ingest_raw_id"):
