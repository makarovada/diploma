...
def test_activate_rollback_and_active_rules(monkeypatch: pytest.MonkeyPatch) -> None:
    conn = MagicMock()
    conn.execute.side_effect = [
        _Rows([{"id": 10, "workspace_id": 1, "source_type": "google_sheet", "stream_name": "orders"}]),  # profile
        _Rows([{"id": 20, "status": "published"}]),  # version
        _Rows([{"ok": 1}]),  # deactivate scope
        _Rows([{"id": 10, "is_active": True, "active_version_id": 20, "workspace_id": 1, "source_type": "google_sheet", "stream_name": "orders", "profile_name": "default", "updated_at": "t", "updated_by": "seed"}]),  # activate row
        _Rows([{"workspace_id": 1, "source_type": "google_sheet", "stream_name": "orders", "profile_name": "default", "rules_json": {"a": 1}, "version": 2}]),  # rollback src
        _Rows([{"rules_json": {"k": "v"}}]),  # get_active_source_rules
        _Rows([{"source_type": "google_sheet", "stream_name": "orders", "rules_json": {"k": "v"}}]),  # all active rules
    ]
    ...
    assert mp.get_active_source_rules(conn, workspace_id=1, source_type="google_sheet", stream_name="orders") == {"k": "v"}
    all_rules = mp.get_all_active_source_rules(conn, workspace_id=1)
    assert all_rules[("google_sheet", "orders")] == {"k": "v"}
